...
def rs_stat():
    ...
    if rs_name != '':
        my_host = args.mongo_host + ':' + str(args.mongo_port)
        for member in rstat['members']:
            if member['name'] == my_host:
                if member['stateStr'] == 'PRIMARY':
                    status = 'Primary'
                elif member['stateStr'] == 'SECONDARY':
                    status = 'Secondary'
                else:
                    status = member['stateStr']  #  显示其他状态，如ARBITER, RECOVERING等
                break
        hd = '[' + rs_name + ' / ' + status + ']'
    else:
        if admin.command('isMaster').get('ismaster', False):  # 如果不是副本集，则判断是否是独立节点
            status = 'StandAlone'
        else:
            status = 'Invalid Server'
        hd = '[' + status + ']'
